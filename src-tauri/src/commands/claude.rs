...
pub async fn run_grooming_agent(ticket_text: String) -> Result<String, String> {
    let (client, api_key) = claude_client()?;
    ...
    complete(&client, &api_key, HAIKU, system, &user, 1500).await
}
...
pub async fn run_triage_turn(
    ...
    complete_multi(&client, &api_key, HAIKU, &system, &history_json, 800).await
}
...
pub async fn finalize_implementation_plan(
    ...
    complete(&client, &api_key, HAIKU, &system, &user, 2000).await
}
...
pub async fn review_pr(review_text: String) -> Result<String, String> {
    ...
    complete(&client, &api_key, HAIKU, system, &user, 4000).await
}
...
pub async fn generate_workload_suggestions(workload_text: String) -> Result<String, String> {
    ...
    complete(&client, &api_key, HAIKU, system, &user, 1024).await
}
...
pub async fn assess_ticket_quality(ticket_text: String) -> Result<String, String> {
    ...
    let user = format!(
        "Assess this ticket for sprint readiness:\n\n{ticket_text}"
    );

    complete(&client, &api_key, HAIKU, system, &user, 1500).await
}

/// Generate a daily standup briefing from pre-compiled team activity text.
/// The caller formats the activity data; this command handles the Claude call.
#[tauri::command]
