...
export function makePlanNode(ctx: PipelineGraphContext) {
  return async function planNode(
    ...
    // `replan_check` after the prior plan failed per-file post-write
    // verification (files missing, empty, etc on disk). Prepend a REVISE
    // preamble so the model produces a *revised* plan rather than
    ...
    if (revision) {
      ...
      updates.verificationOutput = undefined;
    }
    return updates;
  };
}
